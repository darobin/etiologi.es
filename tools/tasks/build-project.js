...
function task(cmd, source, target) {
    return new Promise(function (resolve, reject) {
        browserify(source, { transform: ["babelify"] }).transform(babelify).bundle().on("error", reject).pipe(fs.createWriteStream(target).on("finish", resolve));
    });
}
...
function buildProject(source, target) {
    return new Promise(function (resolve, reject) {
        fs.readJson(source, function (err, data) {
            ...
            var proms = [];
            if (data.browserify) data.browserify.forEach(function (src) {
...
module.exports = function (sourceDir, targetDir, tree) {
    var proms = [];
    for (var k in tree) {
        if (!tree[k].isDirectory() && bn(k) === "project.json") proms.push(buildProject(jn(sourceDir, k), jn(targetDir, k)));
    }return Promise.all(proms);
};